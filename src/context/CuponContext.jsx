...
const CuponProvider = ({children})=>{
    ...
    const descargarCuponComoImagen = async (cuponId) => {
        const elemento = document.querySelector(`[data-swapy-slot="${cuponId}"]`);
        if (!elemento) return;
        ...
        const canvas = await html2canvas(elemento, {
            ...
            // Ignorar botones de editar/eliminar
            return element.classList.contains('Cupon_editorCupon');
            }
        });
        
        const link = document.createElement('a');
        link.download = `cupon-${cuponId}.png`;
